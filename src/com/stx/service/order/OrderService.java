...
public class OrderService {
	 ...
	 * @return
	 */
	public List<Order> queryByMerchantAndStatus(Long merchantId, Integer statusS, Integer statusE){
	 ...
	 * @param counts
	 */
	public Long generateOrder(User loginContext, Merchant merchant, Address address, List<Goods> goodsList, Integer[] counts){
		/** 保存订单 **/
		...
		String userAddr = address.getAddressAttr()+" "+address.getHouseNumber()+" "+address.getRealName()+address.getSex()+"收。  "+address.getMobile();
		...
		/** 动态计价 **/
		Float balance = calculateBalance(merchant.getId(), amount, loginContext.getId());
	...
	public String validateAgain(Long oldOrderId){
		...
		Merchant merchant = merchantService.getMerchantByMerchantId(oldOrder.getMerchantId());
		if(merchant.getOpenFlag()==Constants.FLAG_CLOSE){
			return "商户以下架";
		}
		List<GoodsInfo> goodsInfoList = goodsInfoDao.findGoodsInfoByOid(oldOrderId);
	...
	public List<OrderProcessVo> queryProcessVo(Long userId){
		...
		for(Order order : orderList){
			...
			orderVo.setLogoAddr(merchantService.getMerchantByMerchantId(order.getMerchantId()).getLogoAddr());
			orderVo.setPriceInfo(priceInfoDao.findPriceInfoByOid(order.getId()));
			orderVo.setProcessList(orderProcessDao.findProcessByOid(order.getId()));
	...
	private Float calculateBalance(Long merchantId ,Float amount, Long userId){
		...
		/** 先计算新用户优惠 **/
		List<Strategy> xList = strategyService.listStrategyByType(merchantId,Constants.STRATEGY_XINYONGHU);
		...
		/** 再计算满减活动 **/
		List<Strategy> mList = strategyService.listStrategyByType(merchantId, Constants.STRATEGY_MANJIAN);
		...
		/** 最后计算折扣 **/
		List<Strategy> zList = strategyService.listStrategyByType(merchantId, Constants.STRATEGY_ZHEKOU);
		if(CollectionUtils.isNotEmpty(zList)){
			for(int i = 0 ; i<zList.size(); i++){
				...
				balance = balance + surplus * (1 - (strategy.getDiscount()/10.0f)); 	//优惠价格变化
			}
		}
	 ...
	 * @return
	 */
	public String finishOrder(User loginContext, Long orderId, Integer buzz, String comment){
		/** 订单状态设置为完成，并记录完成时间 **/
		Order order = orderDao.findOrdersById(orderId);
		...
		/** 商品增加销量与按情况增加点赞量 **/
		List<GoodsInfo> goodsInfoList = goodsInfoDao.findGoodsInfoByOid(orderId);
		for(GoodsInfo goodsInfo : goodsInfoList){
			goodsService.addGoodsMonthCount(goodsInfo.getGoodsId(), goodsInfo.getGoodsCount(), buzz);
		}
